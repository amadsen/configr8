...
test('get-matching-paths.js', function (t) {
    t.test('Should find files that match patterns', function (assert) {
        var patterns = [
        ...
        getPaths(name, patterns, env, function (err, r) {
            assert.error(err);
    ...
    t.test('Should substitute {name} to find files that match patterns, including defaults and environment-specific files', function (assert) {
        var patterns = [
    ...
    t.test('Should substitute {name} to find files that match patterns, including defaults but no environment-specific files if environment not set', function (assert) {
        var patterns = [
            posixPath(__dirname, 'test-fs', 'home', '.${name}rc'),
            posixPath(__dirname, 'test-fs', 'cwd', '.${name}rc'),
            posixPath(__dirname, 'test-fs', 'etc', '.${name}rc')
    ...
    t.test('Should substitute {name} with dashes to find only existing files that match patterns, including defaults and environment-specific files', function (assert) {
        var patterns = [
            posixPath(__dirname, 'test-fs', 'home', '.${name}rc'),
            posixPath(__dirname, 'test-fs', 'cwd', '.${name}rc'),
            posixPath(__dirname, 'test-fs', 'etc', '.${name}rc')
