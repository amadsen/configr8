...
function getConfigMerger (name, environment, defaults, overrides, pkgCfg, envCfg, argvCfg) {
    return function mergeConfigs(err, configs, ready) {
        if (err) {
            return ready(err);
        }
        ...
        configs = configs
            .filter( function(cfg){
                if (cfg.error) {
                    //console.error(cfg);
                    errors.push({error: cfg.error, file: cfg.file});
                    return false;
                }
                return true;
            })
